...
// Add this inside the Qwen3TTS implementation section:

bool Qwen3TTS::extract_speaker_embedding(const std::string & reference_audio, std::vector<float> & embedding) {
    std::vector<float> ref_samples;
    int ref_sample_rate;
    if (!load_audio_file(reference_audio, ref_samples, ref_sample_rate)) {
    ...
    if (!audio_encoder_.encode(ref_samples.data(), ref_samples.size(), embedding)) {
...
tts_result Qwen3TTS::synthesize_with_embedding(const std::string & text,
    ...
    return synthesize_internal(text, embedding.data(), params, result);
}

// Add these at the very bottom of the file (before the closing namespace bracket):
